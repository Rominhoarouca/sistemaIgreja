...
class _Contact {
  ...
  final String name;
  final String phone;
  final String kind; // 'lider' | 'visitante' | 'membro'
...
class _LeaderSummary {
  _LeaderSummary({required this.id, required this.name});
...
/// Home do Supervisor — evolução de líderes, células e visitantes sob a sua
/// supervisão + envio de WhatsApp restrito a esse contexto.
...
class _SupervisorDashboardTabState extends State<SupervisorDashboardTab> {
    ...
    try {
      ...
      await Future.wait(
        leadersRaw.map((l) async {
          final leader = _LeaderSummary(
            id: l['id'] as String,
            name: (l['name'] as String?) ?? '',
          );
          leaders.add(leader);
          final leaderPhone = (l['phone'] as String?) ?? '';
          contacts.add(
            _Contact(
              id: leader.id,
              name: leader.name,
          ...
          final cellsResp = await _dio.get(
            '/cells',
            queryParameters: {'leaderId': leader.id},
          );
          final cells =
              ((cellsResp.data as Map<String, dynamic>)['cells'] as List? ?? [])
                  .cast<Map<String, dynamic>>();
          leader.cellCount = cells.length;
            ...
            cells.map((c) async {
              ...
              final visitors =
                  ((results[0].data as Map<String, dynamic>)['data'] as List? ??
                      ...
                      .cast<Map<String, dynamic>>();
              final members =
                  ((results[1].data as Map<String, dynamic>)['members']
                              as List? ??
                          [])
                      .cast<Map<String, dynamic>>();

              leader.visitorCount += visitors.length;
              leader.memberCount += members.length;
      ...
      leaders.sort((a, b) => b.visitorCount.compareTo(a.visitorCount));
      ...
      if (!mounted) return;
      setState(() {
        _error = 'Erro ao carregar dados da supervisão';
        _loading = false;
      });
    }
  }
  ...
  void _openWhatsappSheet() {
    showModalBottomSheet<void>(
      ...
      builder: (_) =>
          _SupervisorWhatsappSheet(leaders: _leaders, contacts: _contacts),
    );
  }
  ...
  void _openLeadersDetail() {
    final leaders = [..._leaders]..sort((a, b) => a.name.compareTo(b.name));
    _openStatDetail(
      title: 'Líderes',
      icon: Icons.record_voice_over_outlined,
      headerValue: '${leaders.length}',
      subtitle: 'Líderes sob a sua supervisão',
      columns: const ['Líder', 'Células', 'Membros', 'Visitantes'],
      rows: leaders
          .map(
            (l) => [
              l.name,
  ...
  void _openCellsDetail() {
    final leaders = _leaders.where((l) => l.cellCount > 0).toList()
      ..sort((a, b) => b.cellCount.compareTo(a.cellCount));
    _openStatDetail(
      title: 'Células',
      icon: Icons.groups_2_outlined,
      headerValue: '$_totalCells',
      subtitle: 'Células por líder da sua supervisão',
      columns: const ['Líder', 'Células', 'Membros', 'Visitantes'],
      rows: leaders
          .map(
            (l) => [
  ...
  void _openVisitorsDetail() {
    ...
    _openStatDetail(
      ...
      subtitle: 'Visitantes das células supervisionadas',
  ...
  void _openMembersDetail() {
    final members = _contacts.where((c) => c.kind == 'membro').toList()
      ..sort((a, b) => a.name.compareTo(b.name));
    _openStatDetail(
      title: 'Membros',
      icon: Icons.group_outlined,
      headerValue: '$_totalMembers',
      subtitle: 'Membros das células supervisionadas',
      columns: const ['Nome', 'Telefone', 'Célula', 'Líder'],
      rows: members
          .map(
            (m) => [
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          // ── Líderes (ranking por visitantes) ─────────────────────────
          AppSectionHeader(title: 'Desempenho dos líderes'),
          const SizedBox(height: AppSpacing.sm),
          ..._leaders.map(
            (l) => Padding(
...
// ═════════════════════════════════════════════════════════════════════════════
// WhatsApp — envio restrito aos líderes supervisionados e suas células
// ═════════════════════════════════════════════════════════════════════════════

class _SupervisorWhatsappSheet extends StatefulWidget {
  const _SupervisorWhatsappSheet({
    required this.leaders,
    required this.contacts,
  });

  final List<_LeaderSummary> leaders;
  final List<_Contact> contacts;

  @override
  State<_SupervisorWhatsappSheet> createState() =>
      _SupervisorWhatsappSheetState();
}

class _SupervisorWhatsappSheetState extends State<_SupervisorWhatsappSheet> {
  String? _leaderFilter; // null = todos
  int _typeFilter = 0; // 0 todos · 1 líderes · 2 visitantes · 3 membros
  final _msgCtrl = TextEditingController();
  final Set<String> _selectedKeys = {};
  bool _sending = false;
  int _sentCount = 0;

  static const _typeLabels = ['Todos', 'Líderes', 'Visitantes', 'Membros'];
  static const _typeKinds = [null, 'lider', 'visitante', 'membro'];
  ...
  List<_Contact> get _filtered => widget.contacts.where((c) {
    if (_leaderFilter != null && c.leaderId != _leaderFilter) return false;
        ...
        children: [
          ...
          Expanded(
            child: ListView(
              ...
              children: [
                // ── Filtro por líder (somente os supervisionados) ──────
                    ...
                    child: Row(
                      children: [
                        FilterChip(
                          ...
                          selected: _leaderFilter == null,
                          showCheckmark: false,
                          onSelected: (_) =>
                              setState(() => _leaderFilter = null),
                        ),
                        for (final l in widget.leaders) ...[
                          const SizedBox(width: AppSpacing.sm),
                          FilterChip(
                            label: Text(l.name),
                            selected: _leaderFilter == l.id,
                            showCheckmark: false,
                            onSelected: (_) =>
                                setState(() => _leaderFilter = l.id),
                          ),
                        ],
                      ],
                    ),
